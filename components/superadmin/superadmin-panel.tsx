...
export function SuperAdminPanel() {
  ...
  const handleAddStore = async () => {
    ...
    for (let i = 0; i < newStore.shopify_brands.length; i++) {
      ...
      if (!brand.brand_name || !brand.store_code || !brand.shopify_store_url || !brand.shopify_token) {
        toast({
          title: "Error",
          description: `Shopify Brand #${i + 1}: All fields are required (Brand Name, Store Code, URL, Token)`,
          variant: "destructive",
        })
        return
      }
  ...
  const handleEditStore = (store: any) => {
    setEditingStore({
      ...
      shopify_brands: store.shopify_brands?.map((b: any) => ({
        ...
        shopify_token: '' // Don't expose existing token
      })) || [{ ...emptyShopifyBrand }]
    })
    setShopifyTestResults({})
    ...
    if (editingStore.shopify_brands) {
      for (let i = 0; i < editingStore.shopify_brands.length; i++) {
        ...
        if (!brand.brand_name || !brand.store_code || !brand.shopify_store_url) {
          toast({
            title: "Error",
            description: `Shopify Brand #${i + 1}: Brand Name, Store Code and URL are required`,
            variant: "destructive",
          })
          return
        }
        // Token is required for new brands (no id)
        if (!brand.id && !brand.shopify_token) {
          toast({
            title: "Error",
            description: `Shopify Brand #${i + 1}: Shopify Token is required for new brands`,
            variant: "destructive",
          })
          return
        }
  ...
  const handleTestShopify = async (url: string, token: string, brandIndex: number = 0) => {
    ...
    setShopifyTestResults(prev => { const next = { ...prev }; delete next[brandIndex]; return next; }) // Clear previous result for this brand
    try {
      const response = await apiClient.testShopifyConnection({ shopify_store_url: url, shopify_token: token })
      if (response.success) {
        setShopifyTestResults(prev => ({ ...prev, [brandIndex]: { success: true, message: response.message || "Shopify connection successful! Credentials are valid." } }))
        toast({
          title: "Connection Successful",
  ...
  // Helper: add a shopify brand to newStore
  const addNewStoreBrand = () => {
    setNewStore({ ...newStore, shopify_brands: [...newStore.shopify_brands, { ...emptyShopifyBrand }] })
  }

  // Helper: remove a shopify brand from newStore
  const removeNewStoreBrand = (index: number) => {
    if (newStore.shopify_brands.length <= 1) return // Must have at least one
    const brands = newStore.shopify_brands.filter((_, i) => i !== index)
    setNewStore({ ...newStore, shopify_brands: brands })
    // Clean up test results
    const newResults = { ...shopifyTestResults }
    delete newResults[index]
    setShopifyTestResults(newResults)
  }

  // Helper: update editing store brand
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                        ...
                        <TableBody>
                          {stores.map((store) => (
                            <TableRow key={store.id}>
                              ...
                              <TableCell className={`${isTablet ? 'max-w-[200px]' : ''}`}>
      ...
      <Dialog open={isEditStoreOpen} onOpenChange={(open) => {
        ...
        <DialogContent className={`${isMobile ? 'max-w-[95vw] max-h-[90vh] overflow-y-auto p-3 sm:p-6' : 'max-w-2xl max-h-[90vh] overflow-y-auto'}`}>
          <DialogHeader>
